...
class FloodRiskPipeline:
    ...
    def __init__(
        ...
        if coast_buffer_dist_m is not None and coastline_path is not None:
            self.coastlinebuffer: Any = CoastlineBuffer(
                dem_path, coastline_path, buffer_dist_m=coast_buffer_dist_m
            )
            self.coast_mask: npt.NDArray[np.bool_] | None = (
                self.coastlinebuffer.create_buffer_mask()
            )
    ...
    def compute_flood_mask(
        self, dem_ds: DatasetReader, water_level: float | None = None
    ...
    def save_flood_raster(
        self, dem_ds: DatasetReader, flooded_mask: npt.NDArray[np.bool_], out_path: Path
    ) -> None:
    ...
    def flooded_polygons_from_mask(
        self, dem_ds: DatasetReader, flooded_mask: npt.NDArray[np.bool_]
    ) -> gpd.GeoDataFrame:
        ...
        shapes = features.shapes(
            flooded_mask.astype(np.uint8), mask=flooded_mask, transform=transform
        )
